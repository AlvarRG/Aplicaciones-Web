...
class FormularioLogin extends Formulario
{
    ...
    protected function procesaFormulario(&$datos)
    {
		...
		//Si no hay errores, comprobamos el login
        if (count($this->errores) === 0) {
            $usuario = Usuario::login($nombreUsuario, $password);
			//Si el login coincide asignamos los valores a la sesión
            if (!$usuario) {
                ...
                $_SESSION['esCamarero'] = $usuario->tieneRol(Usuario::CAMARERO_ROLE);
                $_SESSION['esCocinero'] = $usuario->tieneRol(Usuario::COCINERO_ROLE);
            }
        }
    }
}
